Feature: Account testing

  Scenario: Customer registers in dtu pay
    Given a customer with bank account number "replace-with-valid-customer-bank-acc-number"
    And the bank account number is valid
    When the user registers in dtu pay
    Then the user successfully registers

  Scenario: Merchant registers in dtu pay
    Given a merchant with bank account number "replace-with-valid-merchant-bank-acc-number"
    And the bank account number is valid
    When the user registers in dtu pay
    Then the user successfully registers

  Scenario: Customer registers in dtu pay with invalid account
    Given a customer with bank account number "invalid-customer-bank-acc-number"
    And bank account did not exist
    When the user registers in dtu pay
    Then the user successfully registers

  Scenario: Merchant registers in dtu pay with invalid account
    Given a merchant with bank account number "invalid-merchant-bank-acc-number"
    And bank account did not exist
    When the user registers in dtu pay
    Then the user fails to register registers